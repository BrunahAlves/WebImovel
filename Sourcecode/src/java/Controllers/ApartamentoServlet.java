 ...
 * @author Alisson
 */
@WebServlet(name = "ApartamentoServlet", urlPatterns = {"/Apartamento"})
public class ApartamentoServlet extends HttpServlet {
    ...
    private RequestDispatcher listar(HttpServletRequest request, HttpServletResponse response){
        return request.getRequestDispatcher("WEB-INF/Apartamento/Listar.jsp");
    }
     private RequestDispatcher detalhar(HttpServletRequest request, HttpServletResponse response){
        ...
        return request.getRequestDispatcher("WEB-INF/Apartamento/Detalhar.jsp");
    }
            
    private RequestDispatcher cadastrar(HttpServletRequest request, HttpServletResponse response){
        return request.getRequestDispatcher("WEB-INF/Apartamento/Cadastrar.jsp");
    }
    ...
    private RequestDispatcher inserir(HttpServletRequest request, HttpServletResponse response){
        ...
        //instanciar o imovel
        Models.Apartamento imovel = new Models.Apartamento(metroQuadrado, preco, endereco, andar, numGaragem, numQuartos, numBanheiro);
        ...
        //FakeStorage.Apartamento.List.add(new Models.Apartamento(metroQuadrado, preco, endereco, andar, numGaragem, numQuartos, numBanheiro));
        FakeStorage.Apartamento.List.add(imovel);
    ...
    private RequestDispatcher editar(HttpServletRequest request, HttpServletResponse response){
       int metroQuadrado = Integer.parseInt(request.getParameter("metroQuadrado"));
        ...
        Models.Apartamento user = getById(Integer.parseInt(id));
    ...
    private RequestDispatcher remover(HttpServletRequest request, HttpServletResponse response){
        ...
        FakeStorage.Apartamento.List.remove(getById(id));
        return listar(request, response);
    }
    
    private Models.Apartamento getById(int id){
        Models.Apartamento user = null;
        for(Models.Apartamento u : FakeStorage.Apartamento.List){
            if(u.getId() == id){
                user = u;
                break;
            }
